...
class DrugBankLoader:
    def __init__(
        ...
        bucket_name: str = "drugbank-data-83",
        file_key: str = "full database.xml",
            ...
            "targets":     [
                {
                    ...
                    "uniprot": polypeptide_id(t),
                    "actions": [a.text for a in t.findall(f".//{ns}action")]
                }
                for t in elem.findall(f".//{ns}target")
            ],
            "enzymes":     [
                {
                    "name": e.findtext(f"{ns}name"),
                    "uniprot": polypeptide_id(e)
                }
                for e in elem.findall(f".//{ns}enzyme")
            ],
            "protein_binding": find("protein-binding"),
            "interactions": [
                {
                    "drugbank_id": i.findtext(f"{ns}drugbank-id"),
                    "name": i.findtext(f"{ns}name"),
                    "description": i.findtext(f"{ns}description")
                }
                for i in elem.findall(f".//{ns}drug-interaction")
            ],
        }
